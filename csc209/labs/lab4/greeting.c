...
int main(int argc, char **argv) {
    ...
    // Your code goes here
    char whole_sentence[strlen(argv[1]) + strlen(name) + 2];
    strcpy(whole_sentence, argv[1]);
    strcat(whole_sentence, " ");
    strcat(whole_sentence, name);
    whole_sentence[strlen(argv[1]) + strlen(name) + 1] = '\0';

    strncpy(greeting, whole_sentence, 20);
    greeting[19] = '\0';
