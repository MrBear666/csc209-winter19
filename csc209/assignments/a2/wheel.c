...
char **prune_word_list(char **words, int len, int *words_remaining) {
    ...
    // get the num of word with length len
    while (words[i] != NULL) {
    ...
    *words_remaining = wd_count;

    // get the pruned_list
    pruned_list = malloc(sizeof(char *) * (wd_count + 1));
    if (pruned_list == NULL) {
        perror("malloc");
        exit(1);
    }
    ...
    if (wd_count > 0) {
        while (words[i] != NULL) {
            if (strlen(words[i]) == len) {
                pruned_list[acc] = words[i];
                acc ++;
            }
            i ++;
        }
    }
    pruned_list[wd_count] = NULL;
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    do {
        ...
        // get the stdin len_.
        len_ = strtol(input_buffer, &p_rest, 10);

        // if len_ not valid
        if (len_ > MAX_WORD_LENGTH || *p_rest != '\n') {
            printf("There are no words of that length.\n");
        }
        else { // len_ valid
            pruned_list = prune_word_list(words, len_, &words_remaining);
            // have words with length len_
...
void play_round(char **words) {
    Family *famlist = NULL, *biggest_fam; //q:这啥意思？
    char input_buffer[BUF_SIZE];
    char **word_list = NULL;
    int len, i, found;
    int guesses = 0;
